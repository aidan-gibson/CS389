...
float sum(const uintmax_t size,const int8_t* arr){
  float total = 0.0;
...
int8_t* generate_random_list(unsigned int size, int8_t bound){
//  random_device rd;
//  mt19937 gen(rd()); //if i wanted to seed w random device instead of size
  mt19937 gen(size);
  uniform_real_distribution<int8_t> dist(-bound, bound);
...
void update_coords(int8_t* xs, int8_t* ys, int8_t* zs, const int8_t* vx, const int8_t* vy, const int8_t* vz){
...
//  generate_random_list test code
  ...
  int8_t* xs = generate_random_list(size, static_cast<int8_t>(1000.));
  int8_t* ys = generate_random_list(size, static_cast<int8_t>(1000.));
  int8_t* zs = generate_random_list(size, static_cast<int8_t>(1000.));
  const int8_t* vx = generate_random_list(size, 1.);
  const int8_t* vy = generate_random_list(size, 1.);
  const int8_t* vz = generate_random_list(size, 1.);
//  xs test code
  //  for (size_t i = 0; i<size; i++){
  //    cout << i << " ";
  //    cout << xs[i] << "\n";
